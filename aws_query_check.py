...
def ex_sql(sql, ofile, params):
    ...
    fields = ['PID','User_Name','diff_minutes','diff_hours']
    ...
    add_header(ofile,fields)

def add_header(ofile,fields):
    file_size = os.path.getsize(ofile)
    print("File Size is :", file_size, "bytes")
    
    if file_size != 0:
        file = pd.read_csv(ofile,header=None)
        #print("Thease are the records of file")
        #print(file)
        file.to_csv(ofile, header=fields, index=False)
    else:
        pass
